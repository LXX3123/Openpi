...
def draw_3d_arrow_on_image(base_dir, data_point, output_path, debug_mode=False):
    """
    在图像上绘制3D箭头和坐标轴。
    ...
    # 使用图像尺寸相关的固定值，而不是依赖direction_vector
    img_diagonal = np.sqrt(img_width**2 + img_height**2)
    
    # 绝对固定值控制坐标轴长度 - 大幅减小Y和Z轴长度
    x_length = img_width * 0.5    # X轴长度为图像宽度的50%，保持不变
    y_length = img_width * 0.2    # Y轴长度减小为图像宽度的1%
    z_length = img_height * 0.2   # Z轴长度减小为图像高度的1%

    # X轴 (黄色, 沿matplotlib的Y轴绘制, 指向屏幕内)
    ax.plot([0, 0], [0, x_length], [0, 0], color='yellow', linewidth=2)
    ax.text(0, x_length, 0, 'X', color='yellow')
    # Y轴 (绿色, 沿matplotlib的X轴绘制, 指向右)
    ax.plot([0, y_length], [0, 0], [0, 0], color='g', linewidth=2)
    ax.text(y_length, 0, 0, 'Y', color='g')
    # Z轴 (蓝色, 沿matplotlib的Z轴绘制, 指向上)
    ax.plot([0, 0], [0, 0], [0, z_length], color='b', linewidth=2)
    ax.text(0, 0, z_length, 'Z', color='b')

    # --- FIX: Manually set axis limits to prevent auto-scaling ---
    # Set limits based on the largest axis to maintain consistent scaling.
    # We add a small buffer (e.g., 10%) to ensure text labels are visible.
    limit = x_length * 1.1
    ...
    # --- START: Modified Arrow Length Calculation ---
    # 计算原始向量的范数（长度）
    original_length = np.linalg.norm(direction_vector)

    # 使用更稳定的方法，将箭头长度直接与绘图区域的对角线长度关联
    # 这可以确保无论数据范围如何，箭头都有一个视觉上一致的大小
    
    # 1. 定义视觉上的最小和最大长度（占绘图区对角线的百分比）
    min_len_ratio = 0.50  # 最小箭头长度为对角线的 50%
    max_len_ratio = 0.65  # 最大箭头长度为对角线的 65%

    # 2. 获取绘图区的范围来计算对角线长度
    x_range = ax.get_xlim()[1] - ax.get_xlim()[0]
    y_range = ax.get_ylim()[1] - ax.get_ylim()[0]
    z_range = ax.get_zlim()[1] - ax.get_zlim()[0]
    plot_diagonal = np.sqrt(x_range**2 + y_range**2 + z_range**2)

    # 3. 计算实际的最小和最大像素长度
    min_len = plot_diagonal * min_len_ratio
    max_len = plot_diagonal * max_len_ratio

    # 4. 使用 tanh 函数进行平滑映射
    k = 0.05 # k值控制饱和速度，可以根据你的数据范围微调
    compressed_length = min_len + (max_len - min_len) * np.tanh(k * original_length)

    # 计算方向向量的单位向量
    if original_length > 0:
        unit_vector = direction_vector / original_length
    else:
        unit_vector = np.array([1, 0, 0])  # 默认方向
        
    # 缩放为压缩后的所需长度
    scaled_vector = unit_vector * compressed_length
    # --- END: Modified Arrow Length Calculation ---
    
    arrow = Arrow3D(
        [0, scaled_vector[1]],  # Y-coord on matplotlib's X-axis
        [0, scaled_vector[0]],  # X-coord on matplotlib's Y-axis
        [0, scaled_vector[2]],  # Z-coord on matplotlib's Z-axis
        mutation_scale=15,
        lw=2,
        arrowstyle="-|>",
        color="red"
    ...
    # 7. 合成图像
    if debug_mode:
        # 调试模式：只保存坐标轴和箭头，不显示背景
        cv2.imwrite(output_path, arrow_img_rgba)
    else:
        # 正常模式：将箭头叠加到背景上
        # 调整箭头/坐标轴图像大小以匹配背景
...
if __name__ == '__main__':
    ...
    json_path = '/home/adminroot/lxx/openpi/code/openpi/test/infer/inferred_trajectory.json'
    output_dir = os.path.join(base_directory, 'image_ep0_with_arrow')
    ...
    if not os.path.exists(json_path):
        ...
        if episode_data:
            for i, frame_data in enumerate(episode_data):
                ...
                # 调用函数处理并保存图像
                draw_3d_arrow_on_image(base_directory, frame_data, output_filepath, debug_mode=True)

            # 所有图像处理完毕后，创建视频
            video_output_path = os.path.join(base_directory, 'trajectory_video_ep0.mp4')
...
# 1.  **保存代码**: 将上面的代码块内容保存到您的文件 `trajectory_video.py` 中。
# 2.  **安装依赖**: 您可能需要安装 `matplotlib` 和 `opencv-python`。在终端中运行以下命令：
#     ```bash
#     pip install matplotlib opencv-python numpy
#     ```
# 3.  **执行脚本**: 在终端中，导航到 `trajectory_video.py` 所在的目录并运行它：
#     ```bash
#     python trajectory_video.py
#     ```

# 执行后，您应该会在 `/hpc2hdd/home/zlin810/TestData/drone/UAV-Flow/trajectory_video/` 目录下找到一个名为 `wrist_image_with_arrow_ep0_frame_0.png` 的新图片。这张图片会在原图的中央显示一个青色的三维箭头以及红绿蓝（X,Y,Z）三色坐标轴。# filepath: /hpc2hdd/home/zlin810/TestData/drone/UAV-Flow/trajectory_video/trajectory_video.py
